---
title: "LeandingClub DataExploration"
author: "Vijay K Ummadisetty"
...
loan$Ntotal_acc <-as.factor(cut(loan$total_acc, 5, labels = c("cl1","cl2","cl3","cl4","cl5")))
loan$Npymnt_plan <- as.factor(loan$pymnt_plan)
=======
```{r cars}
summary(cars)
>>>>>>> ce47590a414895b55f2878a43237f7b162aacb90
```
...
mosaic(~Nint_rate+Nterm, 
       data=loan, 
       gp=shading_max, main="Mosaic Plot Interest Rate Vs Term",
       split_vertical=T)
...
ggplot(loan, aes(Nverification_status, ..count..,fill=loan$Npurpose ) ) +
  ...
  coord_flip()+
  facet_grid( Nloan_status ~ . )+labs(x="Count",y="Verification Status",title="No.of Loans per Purpose Grouped by Verification Status")

ggplot(loan, aes(x = factor(""), fill = purpose) ) +  geom_bar() + coord_polar(theta = "y") +
  scale_x_discrete("")+
  facet_grid(loan$grade ~ .)+labs(x="Count",title="No.of Loans per Purpose Grouped by Grade")



ggplot(loan,aes(loan$Nemp_length, loan$loan_amnt))+geom_jitter()+facet_grid(loan$Nannual_inc ~ .)+labs(x="Employment Length",y="Loan Amount",title="Loan amount per Employment Length")
=======
